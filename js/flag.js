(function (G) {
	...
	function D(e, a, d) {
		var g = "", c = a / 8, b, f, k;
		for (b = 0; b < c; b += 3)for (f = b + 1 < c ? e[b + 1 >>> 2] : 0, k = b + 2 < c ? e[b + 2 >>> 2] : 0, k = (e[b >>> 2] >>> 8 * (3 - b % 4) & 255) << 16 | (f >>> 8 * (3 - (b + 1) % 4) & 255) << 8 | k >>> 8 * (3 - (b + 2) % 4) & 255, f = 0; 4 > f; f += 1)8 * b + 6 * f <= a ? g += "ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789+/".charAt(k
				>>> 6 * (3 - f) & 63) : g += d.b64Pad;
		return g
	}
		...
		switch (e) {
			...
			case"TEXT":
				d = function (d, c, b) {
					...
					c = b || 0;
					q = c >>> 3;
					if ("UTF8" === a)for (h = 0; h < d.length; h += 1)for (b = d.charCodeAt(h), e = [], 128 > b ? e.push(b) : 2048 > b ? (e.push(192 | b >>> 6), e.push(128 | b & 63)) : 55296 > b || 57344 <= b ? e.push(224 | b >>> 12,
									128 | b >>> 6 & 63, 128 | b & 63) : (h += 1, b = 65536 + ((b & 1023) << 10 | d.charCodeAt(h) & 1023), e.push(240 | b >>> 18, 128 | b >>> 12 & 63, 128 | b >>> 6 & 63, 128 | b & 63)), m = 0; m < e.length; m += 1) {
						p = l + q;
...
function shaflag(hex, elements) {
	var p = 0;
	$(elements).css({
		color: "white", background: "linear-gradient(to right, " + $.map(hex.match(/....../g), function (e) {
			var r = "#" + e + " " + p + "%";
			p += 100 / 6;
			return r + ", #" + e + " " + p + "%";
		}).join(", ") + ")"
	}).prepend($("<div id='codebox' style='float: right'/>").text("+" + hex.match(/....$/)[0]));
}
function createSHAFlag() {
	var sha1 = new jsSHA("SHA-1", "TEXT");
	var flag = document.getElementById('flag');
	if ((flag) && (flag.dataset.shainput)) {
	...
	for (var i = 0, max = flags.length; i < max; i++) {
		if (flags[i].dataset.input) {
			var sha1 = new jsSHA("SHA-1", "TEXT");
			sha1.update(flags[i].dataset.input);
			var hex = sha1.getHash("HEX");
			shaflag(hex, flags[i])
		}
	}
	var flags = document.getElementsByClassName('shaflag-inline');
	for (var i = 0, max = flags.length; i < max; i++) {
		if (flags[i].dataset.input) {
			var sha1 = new jsSHA("SHA-1", "TEXT");
			sha1.update(flags[i].dataset.input);
